...
# Root helper for contrib/deploy-worker.sh.
#
# Reads base64-encoded secret environment values on stdin, injects them
# into systemd's runtime manager environment, restarts loupe-worker, then
...
service_name="loupe-worker.service"
restart=0
keep_manager_env=0
status_lines=20
...
usage() {
	cat >&2 <<'EOF'
usage: loupe-worker-runtime-secrets [options]

Options:
  --service NAME          systemd unit to restart (default: loupe-worker.service)
...
if [ "$(id -u)" -ne 0 ]; then
	echo "error: loupe-worker-runtime-secrets must run as root" >&2
	exit 1
fi
...
set_runtime_env() {
	...
	case "$name" in
		LOUPE_WORKER_CA_CERT_PEM|LOUPE_WORKER_CERT_PEM|LOUPE_WORKER_KEY_PEM)
